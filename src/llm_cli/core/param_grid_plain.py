...
from llm_cli.core.param_grid_layout import truncate
...
def _render_value_markup(cell: ParamCell, theme: ParamGridTheme) -> str:
    return f"{theme.rich(cell_state(cell))}{escape(cell.value)}[/]"
...
def run_param_grid_plain(
    ...
    *,
    ...
    while True:
        ...
        legend = (
            f"{theme.rich('hint')}"
            f"Number = edit · A = advanced · B = back · S = save · X = abort[/]"
        )
        con.print(legend)
        ...
        if cell.description:
            con.print(f"{theme.rich('dim')}{escape(cell.description)}[/]")
        if cell.hint:
            con.print(f"{theme.rich('hint')}Suggestion: {escape(cell.hint)}[/]")
        ...
        try:
            ...
            con.print(f"{theme.rich('error')}{escape(str(exc))}[/]")
