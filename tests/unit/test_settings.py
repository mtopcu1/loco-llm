...
def test_key_registry_has_required_keys() -> None:
    ...
    assert KEY_REGISTRY["data_root"]["default"] == "~/.loco"
...
def test_settings_path_defaults_to_loco_data_home(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("LOCO_HOME", str(tmp_path / "home"))
    assert settings_path() == (tmp_path / "home" / "config.yaml").resolve()


def test_settings_path_honors_loco_llm_data(monkeypatch, tmp_path) -> None:
    data = tmp_path / "custom-data"
    monkeypatch.setenv("LOCO_HOME", str(data))
    assert settings_path() == data / "config.yaml"


def test_load_settings_missing_file_returns_empty_dict() -> None:
...
def test_load_settings_rejects_unknown_keys(tmp_path, monkeypatch) -> None:
    data = tmp_path / "data"
    data.mkdir()
    (data / "config.yaml").write_text("oops: yes\n", encoding="utf-8")
    monkeypatch.setenv("LOCO_HOME", str(data))
...
def test_save_settings_round_trip(tmp_path, monkeypatch) -> None:
    ...
    save_settings({"data_root": "~/.loco", "repo_root": "/some/repo"})
    assert load_settings() == {"data_root": "~/.loco", "repo_root": "/some/repo"}


def test_save_settings_creates_parent_dirs(tmp_path, monkeypatch) -> None:
    deep = tmp_path / "deep" / "data"
    monkeypatch.setenv("LOCO_HOME", str(deep))
    save_settings({"data_root": "~/.loco", "repo_root": "/r"})
    assert (deep / "config.yaml").is_file()


def test_save_settings_rejects_unknown_keys() -> None:
...
def test_resolve_expands_tilde(monkeypatch, tmp_path) -> None:
    ...
    out = resolve({"data_root": "~/.loco", "repo_root": "~/r"})
    assert out.data_root == tmp_path / ".loco"
    assert out.repo_root == tmp_path / "r"
    assert out.runtimes_dir == tmp_path / ".loco" / "runtimes"


def test_resolve_uses_default_for_data_root_when_missing() -> None:
    out = resolve({"repo_root": "/r"})
    assert out.data_root == Path("~/.loco").expanduser()
...
def test_ensure_data_dirs_creates_all_resolved_dirs(tmp_path) -> None:
    ...
    assert (tmp_path / "dr" / "configs").is_dir()
    assert (tmp_path / "dr" / "state").is_dir()
    ...
    assert (tmp_path / "dr" / "install").is_dir()
