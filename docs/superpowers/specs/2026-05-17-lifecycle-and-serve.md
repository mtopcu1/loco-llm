...
- One verb (`llm serve <config>`) that works in three modes via flags.
- A single `llm status` that tells the truth across all three modes.
- A single `llm switch <config>` that preserves the active mode.
- A single managed systemd unit (`llm.service`), rewritten per config — no "default config pin," no `active.yaml`.
- `llm stop` and `llm logs` that don't care which mode is active.
...
- **No custom systemd unit management.** Power users with hand-written units run `systemctl` themselves; the CLI knows about exactly one unit: `llm.service`.
- **No daily-driver pin.** `state/active.yaml` is not introduced. `llm default` is not introduced.
- **No log rotation.** Per-session append for now; cleanup is `rm`.
- **No `llm bench` / `llm results`.** Out of scope; separate spec.
...
```text
llm serve <config>                      # background (default); waits for ready; returns
llm serve <config> --foreground         # attached to terminal; tracked in state/running.json
llm serve <config> --systemd            # binds llm.service to <config>; waits for ready; returns

llm stop                                # stops whatever is running (mode-aware)
llm switch <config>                     # stop current + start <config> in the same mode
llm status [--json]                     # not running | mode + config + port + uptime
llm logs [--follow|-f] [--lines|-n N]   # mode-aware tail
```

- `--foreground` / `--systemd` are mutually exclusive (and both implicitly negate "background").
- `llm switch` while in foreground mode errors with: "Foreground sessions can't be switched; Ctrl-C in the original terminal and rerun `llm serve <new>`."

### 5.2 State files

| Path | Format | Writer | Reader | Notes |
|---|---|---|---|---|
| `state/running.json` | JSON object | `llm serve` (all modes), cleared by `llm stop` and by foreground's SIGINT trap | `llm status`, `llm stop`, `llm switch`, `llm logs` | At most one record; missing/empty file = nothing running |
| `state/logs/<config-id>.log` | text, per-session append with timestamped header | `llm serve` fg+bg (stderr+stdout) | `llm logs` (file modes), human `tail` | Not written for systemd mode (use journalctl) |
| `state/history.jsonl` | one JSON object per line | every lifecycle command | future UI; humans | Append-only |
| `~/.config/systemd/user/llm.service` | INI unit file | `llm serve <cfg> --systemd` | systemd | Rewritten per config; survives `llm stop` |
...
```
```json
{ "mode": "systemd", "config_id": "...", "unit": "llm.service", "port": 8000,
  "started_at": "..." }
```
...
{ "ts": "...", "action": "systemd-write", "unit": "llm.service", "config_id": "..." }
{ "ts": "...", "action": "reap-stale", "reason": "pid-gone", "config_id": "..." }
```

### 5.3 Truth model

- **fg, bg:** `running.json` is the truth. We sanity-check by `kill -0 <pid>` on `llm status` / before `llm stop`; if the PID is dead, we clear `running.json` (and log a `reap-stale` event).
- **systemd:** `systemctl --user is-active llm.service` is the truth. `running.json` is a convenience cache; if `is-active` says inactive but we have a systemd-mode `running.json`, we clear the cache.
- A `llm serve <other>` while another is live → reject with a clear hint to use `llm switch` or `llm stop`.

### 5.4 Per-mode mechanics

| Concern | `--foreground` | (default) background | `--systemd` |
|---|---|---|---|
| Spawn | `wsl bash -lc '<env>; cd <repo>; exec runtimes/<rt>/serve.sh --config <cfg>'` in CLI's process group; SIGINT trap clears `running.json` and forwards SIGTERM | `wsl bash -lc 'nohup ... </dev/null >>LOG 2>&1 &'`, capture child PID via `wait` + `$!` echo trick | (re)write `~/.config/systemd/user/llm.service`; `systemctl --user daemon-reload` (only if file changed); `systemctl --user restart llm.service` |
...
| `llm logs` impl | `tail -n N -f? state/logs/<id>.log` | same | `journalctl --user -u llm.service -n N [-f]` |
| `llm stop` | `kill -TERM <pid>`; wait up to 10s; `kill -KILL` if still alive; clear `running.json` | same | `systemctl --user stop llm.service`; clear `running.json` |
...
Rewritten on every `llm serve <cfg> --systemd` (or skipped if byte-identical to what we'd write):
...
- Same as `--foreground` in process model (exec, attach to current process), **but** does not write `state/running.json` (the parent `llm serve --systemd` invocation already did).
...
- Hidden from `llm --help` (`hidden=True` on the Typer option).

Linger is not auto-enabled. `llm doctor` grows a `systemd-linger` check that runs `loginctl show-user --property=Linger` and warns if absent; install hint is `sudo loginctl enable-linger $USER`.
...
- Systemd mode: `systemctl --user stop llm.service`, suggest `journalctl --user -u llm.service -n 50`.
...
(`mode: systemd` swaps `pid:` for `unit: llm.service` and `log:` for `journalctl: journalctl --user -u llm.service`.)
...
| Situation | Behavior |
|---|---|
| `llm stop` when nothing is running | Exit 0, print `nothing running`. (Idempotent stop.) |
| `llm switch <cfg>` when nothing is running | Exit 1, `nothing running; use \`llm serve <cfg>\` instead`. |
| `llm logs` when nothing is running | Exit 1, `nothing running`. |
| `llm serve <cfg>` when same config already live in any mode | Exit 1, `<cfg> already running in <mode>; use \`llm switch\` to change config or \`llm stop\` first`. |
| `llm serve <cfg> --systemd` when same config already systemd-active **and** unit byte-identical | No-op success: `already serving <cfg> via systemd`. No restart. |
| `llm switch <cfg>` to the **same** config | Behaves as a restart (stop + start the same config). Useful after editing the config file. |
...
| `src/llm_cli/commands/serve.py` | `llm serve`, `llm switch`. Dispatches to fg/bg/systemd. |
| `src/llm_cli/commands/lifecycle_cmds.py` | `llm stop`, `llm status`, `llm logs`. |
...
- `llm serve <cfg>` (bg) against stub-runtime: returns within readiness window, `llm status` shows `mode: background`, then `llm stop` terminates.
- `llm serve <cfg> --foreground` started in a subprocess thread; `llm status` from another invocation sees it; `llm stop` clears it.
- `llm switch` in bg: stop+start happens; new config in `running.json`.
- Conflict: `llm serve` when one is up → exit 1, hint.
- Port collision: pre-bind the port in the test, `llm serve` fails fast.
- `llm logs -n 5` reads tail of the right file.
...
- New: `docs/lifecycle.md` — mode comparison table, "how to choose a mode," `llm switch` semantics, troubleshooting (port in use, healthcheck timeout, systemd linger).
- `README.md` — extend CLI commands table with `serve / stop / switch / status / logs`. Update Getting Started: add a one-liner `llm serve stub-runtime__stub-model__default` smoke step.
...
- `docs/superpowers/specs/2026-05-15-localllm-scaffolding-design.md` — add a note at the top: "Lifecycle commands are designed in [2026-05-17-lifecycle-and-serve.md](2026-05-17-lifecycle-and-serve.md); they replace the original sections 7.2/7.3 — no `state/active.yaml`, no `llm default`, single managed systemd unit."
...
- **`llm history`.** `state/history.jsonl` is written but not exposed via CLI yet. Add when there's something to read it for (a UI).
- **`llm restart`.** Not added; `llm switch` to the same config covers it.
- **Hot reload of configs.** No — change a config, then `llm switch <cfg>` (rewrites unit if systemd, restarts otherwise).
